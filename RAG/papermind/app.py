...
def main():
    ...
    if uploaded_files:
        ...
        st.success(f"Ready. Created {len(st.session_state['vector_store']['chunks'])} chunks.")
        ...
        if not st.session_state["vector_store"]["chunks"]:
            ...
            with st.spinner("Thinking..."):
                ...
                if not retrieved or retrieved[0]["score"] < 0.15:
                    answer = "I could not find a good answer in the uploaded documents."
                else:
                    try:
                        answer = rag_logic.ask_groq(question, retrieved)
